...
int open_client_fd(char *hostname, char *port) {
    ...
    if ((rc = getaddrinfo(hostname, NULL, &hint, &list_p)) == 0)
        printf("Error in opening getaddrinfo %s\n", gai_strerror(rc));
    for (p = list_p; p; p = p->ai_next) {
...
int open_listen_fd(char *port) {
    ...
    if ((rc = getaddrinfo(NULL, port, &hint, &list_p)) == 0)
        printf("Error in opening getaddrinfo %s", gai_strerror(rc));
    for (p = list_p; p; p = p->ai_next) {
